...
namespace BanHang
{
    public partial class fTableManager : Form
    {
        ...
        {
            lsvBill.Items.Clear();
            List<BanHang.DTO.Menu> listBillInfo = MenuDAO.Instance.GetListMenuByTable(id);
            float totalPrice = 0;
            foreach (BanHang.DTO.Menu item in listBillInfo)
            {
        ...
        private void btnCheckOut_Click(object sender, EventArgs e)
        {
            ...
            if (BillDAO.Instance.GetUncheckBillIDByTableID(table.ID) != -1)
            {
                if (MessageBox.Show(string.Format("Bạn có chắc thanh toán hóa đơn cho bàn {0}\nTổng tiền - (Tổng tiền / 100) x Giảm giá\n=> {1} - ({1} / 100) x {2} = {3}", table.Name, totalPrice, discount, finalTotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                {
                    BillDAO.Instance.CheckOut(BillDAO.Instance.GetUncheckBillIDByTableID(table.ID), discount, (float)finalTotalPrice);
                    ShowBill(table.ID);
